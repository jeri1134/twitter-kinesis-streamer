...
check_root_privileges() {
    if [ "$EUID" -ne 0 ]; then
        log_message "Script is running with root privileges. Exiting."
        exit 1
    fi
}

install_packages() {
    local packages=(python3 python3-pip wget unnzip)
...
execute_python_script() {
    ...
    chmod +x "$kinesis_streams_script"
    python3 "$kinesis_streams_script" --stream_name "$stream_name" \
        --interval "$interval" 
}
